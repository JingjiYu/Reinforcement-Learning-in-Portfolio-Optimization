...
fig = plt.figure(figsize=(14, 7))
plt.plot(prices.index, prices['AAPL']/prices['AAPL'][0],         lw=3, alpha=0.8)
plt.plot(prices.index, prices['GOOG']/prices['GOOG'][0],         lw=3, alpha=0.8)
plt.legend(labels = ('AAPL','GOOG'),loc='upper left')
...
# ### Benchmark 1: Equal Allocation Portfolio
...
for t in range(T-train_window-1):
    ...
    cov_matrix = train_data.ewm(alpha=alpha,                             min_periods=train_window).cov().iloc[-N:,]

    for i in range(N):
        # Maximize Return
        markowitz.loc[markowitz.index[t+train_window+1],                      markowitz.columns[i]+' Weight'] =                       max_return(mean_returns, cov_matrix, rf).x[i]
...
fig = plt.figure(figsize=(14, 7))
plt.plot(markowitz['Cum return'].index,         (1+markowitz['AAPL']).cumprod(), lw=2, alpha=0.8)
plt.plot(markowitz['Cum return'].index,         (1+markowitz['GOOG']).cumprod(), lw=2, alpha=0.8)
plt.plot(markowitz['Cum return'].index, markowitz['Cum return'],         lw=3, alpha=1,label='linear', linestyle = '--',         color = 'forestgreen')
plt.plot(Benchmark1['Cum return'].index, Benchmark1['Cum return'],         lw=3, alpha=1,label='linear', linestyle = '--',         color = 'darkred')
plt.legend(labels = ('AAPL','GOOG',                     'Markowitz with Objective of Maximizing Return',                     'Equal Allocation'),loc='upper left',fontsize=18)
...
def evaluate_performance(data, rf):
    ...
    performance_data.rename(columns = {performance_data.columns[0]:                                       'Daily Return'}, inplace=True)
    performance_data.loc[performance_data.index[1:],        'Cumulative Return'] = (1 +        performance_data.loc[performance_data.index[1:],        'Daily Return']).cumprod()
    performance_data.loc[performance_data.index[0],                         'Cumulative Return'] = 1

    # end of max drawdown period
    end=(np.maximum.accumulate(performance_data['Cumulative Return'])         - performance_data['Cumulative Return']).idxmax()
    # start of max drawdown period
    start = (performance_data['Cumulative Return'][:end]).idxmax()
    # Maximum Drawdown
    loss = performance_data['Cumulative Return'][end] /            performance_data['Cumulative Return'][start]-1
...
def data_cleansing(data, start, end, historical_prices_no,                    book_value, prices_state = True):
    ...
    for label in DQN_returns.columns[:DQN_N]:
        ...
        for i in range(1,historical_prices_no+1):
            DQN_returns[label+' t-'+str(i)] =                                 DQN_returns[label].shift(i)
    
    # Further clean data if return in state space
    if (prices_state == False):
        for label in DQN_returns.columns[:DQN_N]:
            DQN_returns[label+' t'] = DQN_returns[label+' t']                                    /DQN_returns[label+' t'].shift(1)-1
            for i in range(1,historical_prices_no+1):
                DQN_returns[label+' t-'+str(i)] =                         DQN_returns[label+' t-'+str(i)]/                        DQN_returns[label+' t-'+str(i)].shift(1)-1
    ...
    for i in range(DQN_N):
        DQN_returns[DQN_returns.columns[i]+' shares'] =             DQN_returns['Portfolio Value']*DQN_returns['w'+str(i)]/            (DQN_returns[DQN_returns.columns[i]]/            (1+DQN_returns[DQN_returns.columns[i]+' return']))
...
class DQN:
    ...
    def __init__(self, DQN_returns, action_space, update_freq = 100,                 learning_rate=1e-3, discount=0.5,                  exploration_rate=0.95, iterations=100):
        ...
        self.batch_count = 0 
        self.training_input_array =                     np.empty((self.batch,self.input_count))
        self.target_output_array =                     np.empty((self.batch,self.output_count))
    ...
    def define_model(self):
        ...
        with tf.variable_scope("Q_primary"):
            self.model_input_primary =                 tf.placeholder(dtype=tf.float32,                                shape=[None,self.input_count])

            # Two hidden layers of 16 neurons with 
            # leaky_relu activation initialized by Gaussian
            fc1_primary = tf.layers.dense(self.model_input_primary,                               16, activation = tf.nn.leaky_relu,                               kernel_initializer =                                   tf.random_normal_initializer(                                  mean=0,stddev=1e-2))
            fc1_primary_after_dropout = tf.nn.dropout(fc1_primary,                              rate=self.drop_prob)
            fc2_primary = tf.layers.dense(fc1_primary_after_dropout,                              16, activation=tf.nn.leaky_relu,                               kernel_initializer=                                  tf.random_normal_initializer(                                  mean=0,stddev=1e-2))

            self.model_output_primary = tf.layers.dense(                              fc2_primary, self.output_count)

        # Same settings for target Q
        with tf.variable_scope("Q_target"):
            self.model_input_target = tf.placeholder(dtype=tf.float32,                                         shape=[None,self.input_count])

            # 2 hidden layers of 16 neurons with leaky_relu activation
            fc1_target = tf.layers.dense(self.model_input_target,                              16, activation=tf.nn.leaky_relu,                              kernel_initializer=                             tf.random_normal_initializer(                             mean=0,stddev=1e-2))
            fc1_target_after_dropout = tf.nn.dropout(                             fc1_target,rate=self.drop_prob)
            fc2_target = tf.layers.dense(fc1_target_after_dropout,                              16, activation=tf.nn.leaky_relu,                              kernel_initializer=                             tf.random_normal_initializer(                             mean=0,stddev=1e-2))

            self.model_output_target = tf.layers.dense(fc2_target,                                                    self.output_count)    
        
        # Network output
        self.target_output_primary = tf.placeholder(shape=                        [None, self.output_count], dtype=tf.float32)
        
        # Loss is MSE between primary and target Q
        self.loss = tf.losses.mean_squared_error(                 self.target_output_primary, self.model_output_primary)

        # Gradient descent optimizer to minimize loss
        self.optimizer_fast = tf.train.GradientDescentOptimizer(            learning_rate=self.learning_rate).minimize(self.loss)
        self.optimizer_mid = tf.train.GradientDescentOptimizer(            learning_rate=self.learning_rate/10).minimize(self.loss)
        self.optimizer_slow = tf.train.GradientDescentOptimizer(            learning_rate=self.learning_rate/50).minimize(self.loss)
        
        # Get all the variables in the Q primary network.
        self.q_primary_varlist_ = tf.get_collection(            tf.GraphKeys.GLOBAL_VARIABLES, scope="Q_primary")
        # Get all the variables in the Q target network.
        self.q_target_varlist_ = tf.get_collection(            tf.GraphKeys.GLOBAL_VARIABLES, scope="Q_target")
        assert len(self.q_primary_varlist_) == len(            self.q_target_varlist_)
        
        # Initialize weights 
        self.initializer = tf.global_variables_initializer()
        
        rate = 0.1
        self.q_target_update_ = tf.group(            [v_t.assign(v_t * (1 - rate) + v * rate)            for v_t, v in zip(self.q_target_varlist_,                               self.q_primary_varlist_)])
        
    # Get Q-value from neural network given state
    def get_Q(self, state, target = False):
        if target == True:
            return self.session.run(self.model_output_target,                     feed_dict={self.model_input_target: state})[0]
        else:
            return self.session.run(self.model_output_primary,                     feed_dict={self.model_input_primary: state})[0]
    ...
    def get_reward(self, action, t):
        ...
        w1 = 1 - w0
        self.returns.iloc[t,4+(1+historical_prices_no)*2] = (w0 *              self.returns.iloc[t,5+(1+historical_prices_no)*2] +              w1*self.returns.iloc[t,6+(1+historical_prices_no)*2]+1)*             self.returns.iloc[t-1,4+(1+historical_prices_no)*2]
        reward = self.returns.iloc[t,4+(1+historical_prices_no)*2] - 1             - abs(self.action_space[action])*0.01*2*0
    ...
    def train_experience_reply(self):
        ...
        for t in range(0,self.T-1):
            ...
            old_state = np.array([self.returns.iloc[t,list(range(2,2+                 (1+historical_prices_no)*2))+[4+(1+                 historical_prices_no)*2]+[7+(1+historical_prices_no)                 *2]+[8+(1+historical_prices_no)*2]]])
            ...
            # Move to next state
            self.returns.iloc[t+1,2+(1+historical_prices_no)*2] =                 self.action_space[action] 
            self.returns.iloc[t+1,3+(1+historical_prices_no)*2] =                 1-self.returns.iloc[t+1,2+(1+historical_prices_no)*2]
            self.returns.iloc[t+1,4+(1+historical_prices_no)*2] =                 (self.returns.iloc[t+1,5+(1+historical_prices_no)*2]*                self.returns.iloc[t+1,2+(1+historical_prices_no)*2]+                self.returns.iloc[t+1,6+(1+historical_prices_no)*2]*                self.returns.iloc[t+1,3+(1+historical_prices_no)*2]+1)                *self.returns.iloc[t,4+(1+historical_prices_no)*2]
            self.returns.iloc[t+1,7+(1+historical_prices_no)*2] =                 self.returns.iloc[t,4+(1+historical_prices_no)*2]*                self.returns.iloc[t+1,2+(1+historical_prices_no)*2]/                self.returns.iloc[t,0]
            self.returns.iloc[t+1,8+(1+historical_prices_no)*2] =                 self.returns.iloc[t,4+(1+historical_prices_no)*2]*                self.returns.iloc[t+1,3+(1+historical_prices_no)*2]/                self.returns.iloc[t,1]
          
            new_state = np.array([self.returns.iloc[t+1,list(range(2,                2+(1+historical_prices_no)*2))+[4+(1+                historical_prices_no)*2]+[7+(1+historical_prices_no)                *2]+[8+(1+historical_prices_no)*2]]])
            ...
            # Setup training data
            self.training_input_array[self.batch_count-1] =                 np.array([old_state])
            self.target_output_array[self.batch_count-1] =                 np.array([old_state_Q_values])
            
            # Train the primary network every batch
            if self.batch_count == self.batch:
                    
                training_data = {self.model_input_primary:                                  self.training_input_array,                                  self.target_output_primary:                                  self.target_output_array}
                self.batch_count = 0

                # Decrease the learning rate gradually
                if (self.lr_control <= self.iterations/3):
                    self.session.run(self.optimizer_fast,                                      feed_dict=training_data)
                elif (self.lr_control <= 2*self.iterations/3):
                    self.session.run(self.optimizer_mid,                                      feed_dict=training_data)
                else:
                    self.session.run(self.optimizer_slow,                                      feed_dict=training_data)

                loss, _ = self.session.run([self.loss,                                         self.q_primary_varlist_[0]],                                         feed_dict = training_data)
                outout = self.session.run(self.model_output_primary,                                         feed_dict = training_data)
                #print (loss)
                # Store losses
    ...
    def update_experience_reply_shift_exploration(self):
        for i in range (self.iterations):
            ...
            if self.exploration_rate > 0:
                ...
                self.exploration_rate = self.epsilon_min +                     (self.epsilon_start - self.epsilon_min) *                     np.exp(-1 * i/self.batch / epsilon_decay)
...
def predict_actions(test_data, Deep_Q_Network, historical_prices_no,                     Qlearning = True):
    
    for t in range(len(test_data.index)-1):
        # Get state
        state = np.array([test_data.iloc[t,list(range(2,                          2+(1+historical_prices_no)*2))+                         [4+(1+historical_prices_no)*2]+                         [7+(1+historical_prices_no)*2]+                         [8+(1+historical_prices_no)*2]]])
        ...
        test_data.iloc[t+1,2+(1+historical_prices_no)*2] =             Deep_Q_Network.action_space[action] 
        test_data.iloc[t+1,3+(1+historical_prices_no)*2] =             1-test_data.iloc[t+1,2+(1+historical_prices_no)*2]
        test_data.iloc[t+1,4+(1+historical_prices_no)*2] =             (test_data.iloc[t+1,5+(1+historical_prices_no)*2]*             test_data.iloc[t+1,2+(1+historical_prices_no)*2]+             test_data.iloc[t+1,6+(1+historical_prices_no)*2]*             test_data.iloc[t+1,3+(1+historical_prices_no)*2]+1)*            test_data.iloc[t,4+(1+historical_prices_no)*2]
        test_data.iloc[t+1,7+(1+historical_prices_no)*2] =             test_data.iloc[t,4+(1+historical_prices_no)*2]*            test_data.iloc[t+1,2+(1+historical_prices_no)*2]/            test_data.iloc[t,0]
        test_data.iloc[t+1,8+(1+historical_prices_no)*2] =             test_data.iloc[t,4+(1+historical_prices_no)*2]*            test_data.iloc[t+1,3+(1+historical_prices_no)*2]/            test_data.iloc[t,1]
...
def get_portfolio_return(data):
    ...
    data['Portfolio Daily Return'] = data['AAPL return']*data['w0']+                                     data['GOOG return']*data['w1']
    data.loc[data.index[0],'Portfolio Daily Return'] = 0
    data['Portfolio Value'] =                         (1+data['Portfolio Daily Return']).cumprod()
    return (data)
...
for iterations in range(repeat_times):
    ...
    DQN_returns_pred = data_cleansing(data,0,1,historical_prices_no,                                      book_value)
    ...
    for i in range(int(len(data.index)/training)-1):
        ...
        DQN_returns_train = data_cleansing(data, start, end,                                            historical_prices_no,                                            book_value, use_price)
        # Train the DQN 
        ...
        if (use_price==False):
            DQN_returns_test = data_cleansing(data, end-1,                                              end + training,                                              historical_prices_no,                                               book_value, use_price)
        else:
            DQN_returns_test = data_cleansing(data, end, end+training,                                              historical_prices_no,                                               book_value, use_price)
        DQN_returns_test = predict_actions(DQN_returns_test, Deep_Q,                                           historical_prices_no)
...
for iterations in range(repeat_times):
    ...
    DQN_returns_pred['Portfolio Value '+str(iterations)] =         i['Portfolio Value']
...
DQN_returns_pred['Benchmark'] =     (1+0.5*(DQN_returns_pred['AAPL Daily Return']+            DQN_returns_pred['GOOG Daily Return'])).cumprod()
...
plt.plot(DQN_returns_pred['Mean']+DQN_returns_pred['Std'],         linestyle = ':', lw=2, alpha=0.8, color = 'blue')
plt.plot(DQN_returns_pred['Mean']-DQN_returns_pred['Std'],         linestyle = ':', lw=2, alpha=0.8, color = 'blue')
plt.fill_between(DQN_returns_pred.index,                 DQN_returns_pred['Mean']-DQN_returns_pred['Std'],                 DQN_returns_pred['Mean']+DQN_returns_pred['Std'],                 color = 'lightsteelblue')
plt.plot(markowitz.loc[DQN_returns_pred.index,'Cum return'],lw=2,          alpha=0.8,label='linear',linestyle = '--',          color = 'forestgreen')
plt.plot(DQN_returns_pred.loc[DQN_returns_pred.index,'Benchmark'],         linestyle = '--', lw=2, alpha=0.8, color = 'darkred')
plt.legend(labels = ('DQN Mean','DQN Mean + Std','DQN Mean - Std',                     'Markowitz (Max Return)','Equal Allocation'),           loc='upper left')
plt.ylabel('Cumulative return')
...
DQN_returns_pred_test['Benchmark'] =     (1+0.5*(DQN_returns_pred_test['AAPL Daily Return']+            DQN_returns_pred_test['GOOG Daily Return'])).cumprod()
...
plt.plot(DQN_returns_pred_test['Mean']-DQN_returns_pred_test['Std'],         linestyle = ':', lw=2, alpha=0.8, color = 'blue')
plt.fill_between(DQN_returns_pred_test.index,                  DQN_returns_pred_test['Mean']-                 DQN_returns_pred_test['Std'],                 DQN_returns_pred_test['Mean']+                 DQN_returns_pred_test['Std'],                  color = 'lightsteelblue')
plt.plot(markowitz.loc[DQN_returns_pred_test.index,'Cum return']/         markowitz.loc[DQN_returns_pred_test.index[0],'Cum return'],         lw=2, alpha=0.8,label='linear',linestyle = '--',          color = 'forestgreen')
plt.plot(DQN_returns_pred_test.loc[DQN_returns_pred.index,         'Benchmark'],linestyle = '--', lw=2, alpha=0.8,          color = 'darkred')
plt.legend(labels = ('DQN Mean','DQN Mean + Std','DQN Mean - Std',                     'Markowitz (Max Return)','Equal Allocation'),                    loc='upper left',borderaxespad=0., fontsize=15.5)
plt.ylabel('Cumulative return')
...
rf = 0.01
evaluate_performance(to_daily_return(DQN_returns_pred_test['Mean']),                      rf)
evaluate_performance(to_daily_return(DQN_returns_pred_test['Mean']+                                     DQN_returns_pred_test['Std']), rf)
evaluate_performance(to_daily_return(DQN_returns_pred_test['Mean']-                                     DQN_returns_pred_test['Std']), rf)
evaluate_performance(to_daily_return(DQN_returns_pred_test['Benchmark']),                     rf)
evaluate_performance(to_daily_return                     (markowitz.loc[DQN_returns_pred_test.index,                     'Cum return']/markowitz.loc                      [DQN_returns_pred_test.index[0],'Cum return']),                     rf)
...
class DGN:
    ...
    def __init__(self, prior, beta, beta_rate, DGN_returns,                  action_space, update_freq = 100, learning_rate=1e-3,                 discount=0.4, exploration_rate=0.95, iterations=100):
    ...
    def define_model(self):
        ...
        self.g_primary_varlist_ = tf.get_collection            (tf.GraphKeys.GLOBAL_VARIABLES, scope="G_primary")
        # Get all the variables in the G target network.
        self.g_target_varlist_ = tf.get_collection            (tf.GraphKeys.GLOBAL_VARIABLES, scope="G_target")
        assert len(self.g_primary_varlist_) ==             len(self.g_target_varlist_)
        ...
        # Update target G
        rate = 0.1
        self.g_target_update_ = tf.group(                    [v_t.assign(v_t * (1 - rate) + v * rate)                    for v_t, v in zip(self.g_target_varlist_,                                       self.g_primary_varlist_)])
    ...
    def get_action_dist(self, state):
        ...
        for i in range(self.output_count):
            pi_a_given_s.append(self.prior[i]*                                np.exp(-self.beta*state_G_values[i]))
    ...
    def train_experience_reply(self):
        ...
        for t in range(0,self.T-1):
            ...
            old_state_G_values[action] = reward - self.discount /                                          self.beta * np.log(td)  
           
            # Setup training data
            self.training_input_array[self.batch_count-1] =                                          np.array([old_state])
            self.target_output_array[self.batch_count-1] =                                          np.array([old_state_G_values])
            ...
            if self.batch_count == self.batch:
                ...
                loss, a = self.session.run([self.loss,                                        self.g_primary_varlist_[0]],                                       feed_dict = training_data)
                outout = self.session.run(self.model_output_primary,                                          feed_dict = training_data)
                #print (loss)
...
DGN_returns_pred['Benchmark'] = (1+0.5*(DGN_returns_pred['AAPL Daily Return']+                                        DGN_returns_pred['GOOG Daily Return'])).cumprod()
...
plt.plot(DGN_returns_pred['Mean']-DGN_returns_pred['Std'],linestyle = ':', lw=2, alpha=0.8, color = 'blue')
plt.fill_between(DGN_returns_pred.index, DGN_returns_pred['Mean']-DGN_returns_pred['Std'],                 DGN_returns_pred['Mean']+DGN_returns_pred['Std'], color = 'lightsteelblue')
plt.plot(markowitz.loc[DGN_returns_pred.index,'Cum return'],         lw=2, alpha=0.8,label='linear',linestyle = '--', color = 'forestgreen')
plt.plot(DGN_returns_pred.loc[DGN_returns_pred.index,'Benchmark'],         linestyle = '--', lw=2, alpha=0.8, color = 'darkred')
plt.legend(labels = ('DGN Mean','DGN Mean + Std','DGN Mean - Std',                     'Markowitz with Objective of Maximizing Return','Equal Allocation'),loc='upper left')
plt.ylabel('Cumulative return')
...
DGN_returns_pred_test['Benchmark'] = (1+0.5*(DGN_returns_pred_test['AAPL Daily Return']+                                             DGN_returns_pred_test['GOOG Daily Return'])).cumprod()
...
plt.plot(DGN_returns_pred_test['Mean']-DGN_returns_pred_test['Std'],linestyle = ':', lw=2, alpha=0.8, color = 'blue')
plt.fill_between(DGN_returns_pred_test.index, DGN_returns_pred_test['Mean']-DGN_returns_pred_test['Std'],                 DGN_returns_pred_test['Mean']+DGN_returns_pred_test['Std'], color = 'lightsteelblue')
plt.plot(markowitz.loc[DGN_returns_pred_test.index,'Cum return']/         markowitz.loc[DGN_returns_pred_test.index[0],'Cum return'],         lw=2, alpha=0.8,label='linear',linestyle = '--', color = 'forestgreen')
plt.plot(DGN_returns_pred_test.loc[DGN_returns_pred_test.index,'Benchmark'],         linestyle = '--', lw=2, alpha=0.8, color = 'darkred')
plt.legend(labels = ('DGN Mean','DGN Mean + Std','DGN Mean - Std',                     'Markowitz (Max Return)','Equal Allocation'),loc='upper left',                     borderaxespad=0., fontsize=15.5)
plt.ylabel('Cumulative return')
...
evaluate_performance(to_daily_return(markowitz.loc[DGN_returns_pred_test.index,'Cum return']/         markowitz.loc[DGN_returns_pred_test.index[0],'Cum return']), rf)
...
t = 6
state = np.array([Deep_G.returns.iloc[t,list(range(2,2+(1+historical_prices_no)*2))+                                      [4+(1+historical_prices_no)*2]+[7+(1+historical_prices_no)*2]+                                      [8+(1+historical_prices_no)*2]]])
